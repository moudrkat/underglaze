...
btn.onclick = async () => {
  ...
  try{
    ...
    window.wall._pickLine = async (q, opts, id) => {
      ...
      // NONE has to be a real option or it is never taken. Measured before this
      // was rewritten: "hello", "what are you" and "what is 2+2" all came back
      // with the same sentence about standing four metres away, because the
      // model was picking the least wrong of ten rather than saying none of
      // them answered the question. It is asked whether one answers it first.
      const said = await say(
