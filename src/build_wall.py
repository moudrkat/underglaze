...
DESC = {"cut": "How many numbers it takes to write this tile down. Cosines, Fourier terms, how much data and information it costs to store or describe the pattern, compression, the shortest description, brush strokes against waves.", "kil": "What the kiln and the firing do to it. Heat, fire, the oven, baking, nine hundred degrees, glaze melting, how firing blurs and erases the painted lines, how the cobalt creeps and spreads.", "perc": "When the separate blue flowers join up into one connected shape. The cut-off, the threshold, percolation, why the tile is blue and where the blue stops, whether the ink is one piece or many, whether the flowers touch each other, whether it spans edge to edge.", "eye": "What a person standing in the room actually receives. Distance in metres, standing back, across the room, from the doorway, squinting, eyesight and visual acuity, an arcminute, how much of the fine work never reaches anybody at all.", "chi": "Handedness and mirrors. Which way the tendrils curl, chirality, the wallpaper group p4, quarter turns, reflections, whether the pattern is the same the whole way round, flipping it, turning it upside down.", "frac": "Whether the pattern is a fractal. Zooming in, looking closer, magnification, self-similarity, box dimension, roughness at every scale, detail all the way down, a pattern inside the pattern, going on forever.", "copy": "Who painted this tile and how old it is. The painter, the copyist, three centuries of being copied by hand, its age, how many years it has been here, where it came from, cobalt and blue and white, the onion it is named after, Meissen and China, whether it survives being reproduced again and again.", "ship": "How much of the tile can be replaced before it stops being this tile. What it is, what you are looking at, its identity, what is essential about it, what makes it itself and not another pattern, the ship of Theseus, swapping and shuffling its coefficients.", "attn": "What a machine learning model finds in the tile. Attention, transformers, neural networks, embeddings, software recognising or reading the pattern, what an AI notices and what it misses."}
...
TAG = ['And which of us is talking to a wall here?', 'Just so we are clear on who the wall is.', 'You asked, remember.', 'I did not start this.', 'Say what you like about my conversation, I have never left the room.', 'Anyway. You are still here.', 'No pressure. I have got until the building comes down.', 'That is the sort of thing you learn standing still for ninety years.', 'Do not let me keep you.', 'It is a strange hobby you have, but go on.', 'I would nod, but.', 'You may quote me. I am not going anywhere.', 'One of us has somewhere else to be.', 'This counts as my busiest day.', 'The kettle disagrees, but the kettle always does.', 'I only know nine things, and you have now heard one of them.', 'There is more where that came from. Eight more, to be exact.', 'Ask the tile next to me. She will say the same about herself.', 'None of this was my idea.', 'You did ask.', 'I have had ninety years to think about it.', 'I would shrug.', 'Take it up with whoever fired me.', 'The grout knows more than it lets on.', 'That is one of nine. Pace yourself.', 'Somebody wrote that down so you would not have to.', 'Do not ask the kettle. The kettle exaggerates.', 'I am at my most eloquent on this particular subject.', 'It took a camera and a great deal of arithmetic to get that out of me.', 'Riveting, I know.']
ASIDE = ['You are talking to a wall. I would like that on the record.', 'This is going better than the phrase suggests.', 'Somebody photographed me and now I have opinions.', 'Ninety years on this wall and today is the first time anyone asked.', 'You could be outside. There is a whole valley out there.', 'I am a wall. You are doing most of the work in this conversation.', 'Nine tiles and a kettle. That is the entire social circle in here.', 'There is no polite way to put this: I am grouting.', 'The kettle has heard all of this before.', 'I cannot leave, so take your time.', 'You are the first thing to happen here since the boiler.', 'I answer, which is more than the phrase promised.', 'Nobody in this kitchen has ever asked me twice.', 'The boiler is the only other thing in here with an opinion.', 'Ninety years, and the most exciting event was a leak.', 'Thousands have looked at me. One has asked.', 'You are not the first to talk to a wall. You are the first to wait for the answer.', 'The kettle boils, the fridge hums, and I do this.', 'I was put up straight, which is more than can be said for the shelf.', 'Whatever you were doing before this, it was probably load-bearing.']
...
# Low, on purpose. Deciding that a question is not for this wall is not the
# router's job -- it cannot read, it can only say which of nine subjects a
# sentence is nearest. Deciding is call two's job: it is looking at ten actual
# sentences and can say none of them fits, and then call three writes one. An
# earlier version tried to do the refusing here, with three decoy subjects and
# a margin, and it spent its time taking real questions off real tiles.
# Swept over all 141 questions, with the router comparing against the nine
# tiles and nothing else, and with the nine subjects rewritten so they stop
# overlapping: 0.14 lands 109, against 92 at the 0.20 this page used to run at.
# Higher refuses nonsense better and answers real questions worse; 0.14 is
# where the two stop paying for each other.
THRESHOLD = 0.10
...
/* The tenth. On a phone the wall is two columns and nine will not fill them,
   so it ended on a hard rectangle of mortar that read as a mistake rather than
   as a wall. This is a real piece of the photographed wall with nothing
   measured about it -- no question, no slider, no ? -- and it says so
   underneath rather than pretending to be one of the nine. It exists only
   below 600px, where the grid needs it. */
.tile.dead{display:none}
.tile.dead .face{background:url(wall.jpg) 30% 26%/315% 315%;
 filter:saturate(.72) brightness(1.06); opacity:.74}
.tile.dead .tag{opacity:.8}
.deadnote{flex:0 0 auto;margin:5px 3px 0;font-size:10.5px;line-height:1.35;
 color:var(--grey);font-style:italic}
@media(max-width:600px){.tile.dead{display:flex;flex-direction:column}}
...
.said .think b{font-weight:400;font-style:normal;margin-left:.6em;color:var(--grey)}
...
#bub .x{position:absolute;top:5px;right:9px;color:var(--grey);cursor:pointer;
 font:14px sans-serif;background:none;border:0;padding:2px 4px;line-height:1}
#bub .x:hover{color:var(--ink)}
#bub .x:focus-visible{outline:2px solid var(--warm);outline-offset:1px}
...
<input type="text" id="q" disabled placeholder="press &ldquo;let it think&rdquo; above and you can ask it things"
...
<button class="go" id="speak" title="download both models and run them in this browser, no server and no key">let it think &middot; 506 MB</button>
...
''' + cells + '''
<div class="tile dead" aria-hidden="true">
<div class="pic"><div class="face"><span class="tag">THE TENTH</span></div></div>
<p class="deadnote">Nobody measured me. I am here because nine will not fill two columns.</p>
</div>
</div></div>
...
<div id="sug"></div>
<div id="bub"><button type="button" class="x" onclick="wall.close()" aria-label="close">&#215;</button><h4 id="bubh"></h4>
...
window.wall = {
  ...
  stop(){ stopped=true; clearInterval(this._rt); clearInterval(this._typing);
    this.hint(null);
    // A sweep cut off part way leaves its tile somewhere along its slider, and
    // the wall is meant to be found the way it was left. This is the one place
    // every way of stopping goes through -- the play button pressed twice, a
    // question arriving, the film -- so it is the one place that has to put it
    // back. Measured before it was here: pressing play a second time left all
    // nine sliders off their rest positions and nothing ever moved them back.
    document.querySelectorAll('.tile.speaking').forEach(c => {
      const r = document.getElementById(c.dataset.tile + 'r');
      if(r) this.set(c.dataset.tile, +r.defaultValue); });
  ...
  think(id){
    ...
    // Fifteen seconds of a page saying nothing but "thinking" reads as broken.
    // The first estimate is the one answer that has been timed end to end --
    // 14.5 s, headless, on a laptop with no GPU -- and it is replaced by this
    // visitor's own machine as soon as that machine has answered once.
    this._thinkAt = Date.now();
    const eta = this._lastMs
      ? '(about ' + Math.round(this._lastMs / 1000) + ' s, going by the last one)'
      : '(about 15 s the first time \u2014 it is running on your machine)';
    el.innerHTML = (tag ? '<em class="think">the tile ' + this.place(id) + ' is thinking'
                        : '<em class="think">the wall is thinking')
      + '<i>.</i><i>.</i><i>.</i><b>' + eta + '</b></em>';
    el.classList.add('on');
    return 'thinking';
  },
  label(id){
  ...
  async _sweep(id, ms, atTurn, my, passes){
    ...
    const leg = async (from, to, step) => {
      for(let i = from; step > 0 ? i <= to : i >= to; i += step){
        if(dead()) return true; this.set(id, i); await sleep(ms); }
      return false;
    };
    for(let p = 0; p < passes; p++){
      if(await leg(rest, far, d)) return;
      // The sentence lands at the turn of the first pass and the tile goes on
      // moving underneath it. A line with a still wall under it is half an
      // answer -- so the wait at the far end is long enough to read the
      // sentence twice over, and a tile that is speaking gets more than one
      // pass to say it in.
      if(p === 0 && atTurn) await atTurn();  // the model may take a while; wait
      if(dead()) return;
      await sleep(p === 0 && atTurn ? 2200 : 680);   // to read what arrived
      if(await leg(far, rest, -d)) return;
    }
  ...
  async perform(ms=95){
    if(running || this._asked) return 'busy';
    const my = ++gen;
    stopped = false; running = true;
    const ids = Object.keys(TILE);
    const wall = document.querySelector('.wall');
    for(let n = 0; !stopped && my === gen && n < 30; n++){
      // It does not open as a chorus. Nine tiles moving at once is an
      // animation; one tile moving is a wall with something to say and eight
      // others waiting their turn. So it takes turns first, now and then two
      // at a time, and the chorus arrives a few beats in -- where it reads as
      // a change rather than as the state the page happened to load in.
      const all = n === 2 || (n > 2 && Math.random() < 0.25);
      const pair = !all && n > 2 && Math.random() < 0.25;
      // checked again here and not only in the loop condition: a question can
      // arrive between the two, and then this beat takes the wall back off it
      if(this._asked || my !== gen) break;
      const bag = ids.slice();
      for(let i = bag.length - 1; i > 0; i--){
        const j = Math.floor(Math.random() * (i + 1));
        const swap = bag[i]; bag[i] = bag[j]; bag[j] = swap; }
      // and not the tile that just spoke: picking at random out of nine gave
      // the same one twice in a row often enough to read as the wall being
      // stuck rather than as it choosing
      if(!all && bag[0] === this._lastSolo && bag.length > 1) bag.push(bag.shift());
      const cast = all ? ids : bag.slice(0, pair ? 2 : 1);
      if(!all) this._lastSolo = cast[0];
      // one speaker or none: two tiles moving have no single line between them,
      // so a pair is handled the way the chorus is
      const many = cast.length > 1;
      document.querySelectorAll('.tile').forEach(c => c.classList.remove('speaking'));
      cast.forEach(i => document.querySelector('[data-tile="'+i+'"]').classList.add('speaking'));
      wall.classList.add('busy');
      this._said = many ? [] : cast.slice(); this._at = 0;
      if(many){ const C = WALLLM.CHORUS;
      ...
      await Promise.all(cast.map(id =>
        this._sweep(id, ms, many ? null : () => this.line(cast[0]), my)));
      ...
      if(stopped) break;
      await sleep(many ? 1600 : 1200);
      // Nothing changes under somebody reading it. The sentence and the two
      // links under it are both about the tile speaking now, and both were
      // being replaced mid-read: measured over thirty seconds, "find out more"
      // pointed at three different tiles with nobody touching the page, so it
      // would open a tile the visitor had never been reading about. While the
      // pointer is on that line the whole performance waits.
      while(this._hold && !stopped && my === gen) await sleep(150);
    }
    if(my === gen) running = false;   // putting the wall back is stop()'s job
    return 'done';
  },
  ...
  async ask(q){
    ...
    // whatever the wall was doing on its own is over, and it does not start
    // again by itself once somebody has spoken to it
    gen++;
    this._asked = true;
    ...
    try{
      ...
      // Three of the twelve subjects the router compares against are not tiles:
      // arithmetic and general knowledge, small talk, and questions about the
      // model itself. If one of those is nearest then the question is not about
      // this wall, whatever it scored. Still the model deciding -- it just has
      // somewhere to put what the wall was never measured for, instead of
      // having to name the least wrong tile.
      if(id && sim >= WALLLM.THRESHOLD) return this.act(id);
  ...
  say(text, who){
    ...
    clearInterval(this._typing);
    // whatever the estimate said, this is what it was
    if(text && this._thinkAt){ this._lastMs = Date.now() - this._thinkAt;
                               this._thinkAt = 0; }
  ...
  hint(id){
    ...
    el.innerHTML = (ids.length > 1
      ...
      + 'find out more</a>'
      // Without a model the whole page is this link and the tiles' own ?, and
      // the sliders were reachable only through a bar button whose meaning
      // lived in a tooltip. tryit() has been sitting here unreferenced since
      // it was written; this is what was supposed to call it.
      + '<b>&middot;</b>'
  ...
  async line(id, q){
    ...
    // Whatever this turns into must not be said if the wall has moved on. The
    // performance starts a call, somebody types a question, and twenty-five
    // seconds later the performance's answer lands on top of the question --
    // which is what the film caught: "the wall is thinking" replaced by the
    // last thing it had been saying to itself, under a question nobody had
    // answered yet.
    const mine = gen;
    const speak = (t, who) => (gen === mine) ? this.say(t, who) : null;
    let s = null;
    const st = document.getElementById('mstat');
    if(this._pickLine && q){
      this.think(id);
    ...
    let line = s || ls[0];
    if(line.length < 78 && Math.random() < 0.45){
...
// A panel that covers the wall has to be dismissable the two ways everybody
// tries first. Measured on the deployed page before this: escape did nothing,
// clicking away did nothing, and the x was a span, so it was not reachable by
// keyboard either.
document.addEventListener('keydown', e => {
  if(e.key === 'Escape' && document.getElementById('bub').classList.contains('on'))
    wall.close();
});
document.addEventListener('click', e => {
  const b = document.getElementById('bub');
  if(!b.classList.contains('on') || b.contains(e.target)) return;
  if(e.target.closest && e.target.closest('button')) return;   // let the bar work
  wall.close();
});
document.querySelectorAll('.saidwrap').forEach(z => {
  ['pointerenter','focusin'].forEach(e =>
    z.addEventListener(e, () => { wall._hold = true; }));
  ['pointerleave','focusout'].forEach(e =>
    z.addEventListener(e, () => { wall._hold = false; }));
});
setTimeout(() => { if(!running) wall.perform(); }, 1400);
setInterval(() => {
  if (running || wall._asked || Date.now() - lastTouch < 25000) return;
  lastTouch = Date.now();
  wall.perform();
}, 4000);
...
btn.onclick = async () => {
  ...
  try{
    ...
    // Off the main thread, or the page freezes for the whole of every answer.
    // Twenty-five seconds of generation on wasm was blocking everything: the
    // tile stopped mid-sweep, the caret stopped blinking, and a return key
    // pressed during it sat in the queue until the model had finished. All of
    // which was filmed before anybody noticed, because a frozen screen
    // recording of a still page looks like a still page.
    ...
    // Straight on to call two and call three: which of the tile's ten, and --
    // when none of them fits -- one sentence of its own. A generator's work,
    // which a router cannot do, the same way the generators could not route.
    // Qwen2.5-0.5B and not SmolLM2-135M, because SmolLM2 answered "1" to all
    // eighteen lists of ten and Qwen reached ten distinct lines.
    const wbtn = btn;
    ...
    // The raw generator, so an evaluation can try a prompt against the real
    // model in the real page without a round trip through the whole flow.
    window.wall._say = say;
    ...
    window.wall._pickLine = async (q, opts, id) => {
      // Refusing here is a handover: call two declines the ten written lines
      // and call three writes one instead. Three shapes were measured trying
      // to make it decline, and all three are recorded because none of them
      // worked:
      //
      //   NONE named in the instructions   forced a line onto 12 of the 18
      //     (this one, still shipping)     questions the wall was never
      //                                    measured for; 9 of 9 tile questions
      //                                    answered properly
      //   NONE as an eleventh numbered     11 of 18 forced, and it began
      //     line                           refusing 2 of the 9 the tile does
      //                                    answer. A worse trade.
      //   a yes/no gate before the pick    0 of 18 forced and 8 of 9 tile
      //                                    questions refused -- and probed on
      //                                    its own, the gate answered the same
      //                                    word to every question it was
      //                                    given. Two phrasings said no nine
      //                                    times out of nine, one said yes
      //                                    eight times out of nine. It is
      //                                    answering the template, not the
      //                                    question.
      //
      // So call two cannot yet be made to hand over, and the honest state is
      // the first one: the wall answers its own questions properly and forces
      // a written line onto two thirds of what it was never measured for.
      const numbered = opts.map((l,i) => (i+1) + '. ' + l).join('\\n');
      const said = await say(
        'You are the ' + (NAMES[id] || 'tile') + ' tile on a kitchen wall. '
        + (DESC[id] || '') + ' Below are the only ten things you are able to say. '
        + 'If one of them answers the visitor\\'s question, reply with its number. '
        + 'If none of them answers it, reply NONE. Do not force one. '
        + 'Reply with a single number, or the word NONE.',
        'Question: "' + q + '"\\n' + numbered + '\\nNumber or NONE:', 4);
    ...
    window.wall._improvise = async (q, id) => {
      let t = await say(
        'You are a wall of nine painted tiles in a kitchen in a valley under the '
        + 'Krkono\u0161e, in the Czech Republic, and you have been on that wall since 1885. '
        + 'Your pattern is the onion pattern, Zwiebelmuster, copied by hand for three '
        + 'centuries: from a Chinese bowl to Meissen to Dub\u00ed to a steel printing plate '
        + 'to this kitchen. There is no onion on you anywhere. Somebody photographed you '
        + 'once and measured everything about you off that one photograph. You are dry, '
        + 'plain and a little bored, and you say concrete things -- the kettle, the grout, '
        + 'the light, ninety years of the same room -- never abstract ones. You are not an '
        + 'assistant: no greeting, no offer of help, no talk of your purpose. One short '
        + 'sentence, first person, no numbers. '
