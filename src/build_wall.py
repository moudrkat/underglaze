...
.pic{position:relative;flex:1 1 auto;min-height:0;display:flex;perspective:760px;
 background:url(plaster.jpg) 0 0/512px 512px repeat, linear-gradient(168deg,#ded8cd,#cdc6b8);
 box-shadow:inset 0 0 0 2px rgba(84,68,48,.30),      /* grout squeezed out at the rim */
            inset 0 4px 11px rgba(42,33,23,.40),     /* the tile sat proud of this */
            inset 0 -3px 8px rgba(42,33,23,.24)}
...
/* the tile itself is one loose object hinged on its bottom edge; ? takes it off the wall */
.face{position:absolute;inset:0;z-index:1;display:flex;transform-origin:50% 100%;
 backface-visibility:hidden;
 transition:transform .52s cubic-bezier(.42,.02,.24,1), box-shadow .52s}
/* -84deg leaves the bottom edge of the tile in the frame, tipped out and lit along the
   rim, so it reads as taken off and leaning there rather than as a tile that vanished */
.tile.open .face{transform:rotateX(-84deg) translateY(1%);
 box-shadow:0 15px 24px rgba(60,50,38,.45)}
...
/* Nobody writes on bare render. The sand ate the thin strokes of the serif and
   a text-shadow could not save it, so the writing sits on a patch of limewash
   -- brushed on, soft at the edge, thin enough that the grain still reads
   through it. It shrink-wraps the words, so the rest of the recess stays wall. */
.note{background:rgba(249,245,236,.90);padding:6% 7% 7%;border-radius:2px;
 box-shadow:0 0 0 5px rgba(249,245,236,.62), 0 0 16px 11px rgba(249,245,236,.44)}
...
.tile.open .say{opacity:1;visibility:visible;transition:opacity .3s .18s, visibility 0s}
